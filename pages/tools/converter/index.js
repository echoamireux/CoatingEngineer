...
/* 👇👇👇 删除原来的 fmt，替换为下面这两个函数 👇👇👇 */
  ...
  fmt(num) {
    ...
    // 如果数值过大或过小，使用自定义科学计数法
    if (abs > 100000 || abs < 0.001) {
      let str = num.toExponential(3); // 例如 "5.340e+6"
      let parts = str.split('e');
      // 替换 e 为 ×10^，并去掉指数部分的 + 号
  ...
  parseVal(str) {
    if (!str) return NaN;
    // 把显示的 ×10^ 替换回 e，让 JS 能计算
    let s = str.toString().replace(/×10\^/g, 'e');
    return parseFloat(s);
  },
  /* 👆👆👆 替换结束 👆👆👆 */
